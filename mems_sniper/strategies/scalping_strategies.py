...
class ScalpProBTB(BaseScalpStrategy):
    ...
    def evaluate(self, df: pd.DataFrame, ctx: Dict[str, Any]) -> Optional[StrategyHit]:
        ...
        score = float(np.clip(0.55 + 0.15 * min(vol_ratio / 2, 1), 0, 1))
...
class ScalpSP2L(BaseScalpStrategy):
    ...
    def evaluate(self, df: pd.DataFrame, ctx: Dict[str, Any]) -> Optional[StrategyHit]:
        ...
        bullish_spike = _detect_spike_run(open_, close, high, low, atr_val, n, "bullish",
                                           ...
                                           scan_start=max(0, n - 15), scan_window=15)
        ...
        if bullish_spike is not None:
            ...
            hls = [float(low.iloc[i]) for i in range(spike_end, min(spike_end + 10, n))]
            consecutive_hl = 0
        ...
        if signal is None and bearish_spike is not None:
            ...
            lhs = [float(high.iloc[i]) for i in range(spike_end, min(spike_end + 10, n))]
            consecutive_lh = 0
        ...
        score = float(np.clip(0.52 + hl_bonus + 0.18 * min(vol_ratio / 2, 1), 0, 1))
